...
class CartProduct extends DataClass implements Insertable<CartProduct> {
  ...
  Map<String, Expression> toColumns(bool nullToAbsent) {
    ...
    if (!nullToAbsent || variant_info != null) {
      ...
      if (variant_info == null) {
        _viEncoded = null;
      } else if (variant_info is String) {
        _viEncoded = variant_info as String; // already a JSON string from SQLite
      } else if (variant_info is VariantInfo) {
        _viEncoded = jsonEncode((variant_info as VariantInfo).toJson());
      } else {
        _viEncoded = jsonEncode(variant_info);
      }
      map['variant_info'] = Variable<String?>(_viEncoded);
    }
    return map;
  }
  ...
  factory CartProduct.fromJson(Map<String, dynamic> json,
    ...
    return CartProduct(
      ...
      variant_info: (() {
          final rawVi = json['variant_info'];
          if (rawVi == null) return null;
          try {
            if (rawVi is Map<String, dynamic>) {
              return VariantInfo.fromJson(rawVi);
            } else if (rawVi is String && rawVi.isNotEmpty && rawVi != 'null') {
              final decoded = jsonDecode(rawVi);
  ...
  Map<String, dynamic> toJson({ValueSerializer? serializer}) {
    ...
    return <String, dynamic>{
      ...
      'variant_info': (() {
          if (variant_info == null) return null;
          final ser = serializer!;
          try {
            if (variant_info is VariantInfo) {
              return ser.toJson<Map<String, dynamic>>(variant_info.toJson());
            } else if (variant_info is Map<String, dynamic>) {
              return ser.toJson<Map<String, dynamic>>(Map<String, dynamic>.from(variant_info));
            } else if (variant_info is String &&
                (variant_info as String).isNotEmpty &&
                variant_info != 'null') {
              return ser.toJson<Map<String, dynamic>>(
                  VariantInfo.fromJson(jsonDecode(variant_info as String)).toJson());
            }
          } catch (_) {}
          return null;
        })(),
