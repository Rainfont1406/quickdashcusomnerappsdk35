...
class StoryItem {
  ...
  /// Not final: video stories update this once the real duration is known.
  Duration duration;
  ...
  static StoryItem text({
    ...
    return StoryItem(
      Container(
        ...
        padding: textOuterPadding?? const EdgeInsets.symmetric(
          horizontal: 24,
          vertical: 16,
        ),
        child: Center(
...
class StoryViewState extends State<StoryView> with TickerProviderStateMixin {
  ...
  // Stored so we can re-attach it when the animation controller is recreated
  // after a video reports its real duration.
  ...
  void initState() {
    ...
    // When a video widget reports its actual runtime duration, correct the
    // animation controller so the progress bar matches the video exactly.
    // This eliminates the "black screen overshoot" when a video is shorter
    // than the default 10-second fallback.
    _durationSubscription =
        widget.controller.durationNotifier.listen(_updateCurrentStoryDuration);

    _play();
  }

  // Called when a StoryVideo detects the real video length after initialisation.
  void _updateCurrentStoryDuration(Duration actual) {
    final storyItem = _currentStory;
    if (storyItem == null) return;
    // Skip if the difference is negligible (< 200 ms) to avoid micro-resets.
    if ((storyItem.duration - actual).abs() < const Duration(milliseconds: 200)) return;

    storyItem.duration = actual;

    // Rebuild the animation controller with the real duration, preserving
    // however far the progress bar has already advanced.
    final progress = _animationController?.value ?? 0.0;
    _animationController?.dispose();
    _animationController = AnimationController(duration: actual, vsync: this);
  ...
  void _play() {
    ...
    // Store as a named reference so _updateCurrentStoryDuration can reattach
    // it after recreating the animation controller with the real video duration.
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      ...
      foregroundPainter: IndicatorOval(
        indicatorForegroundColor?? Colors.white.withValues(alpha: 0.9),
        value,
      ),
