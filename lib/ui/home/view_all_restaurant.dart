...
class _ViewAllRestaurantState extends State<ViewAllRestaurant> {
  ...
  getProducts() async {
    ...
    GeoFirePoint center = Geoflutterfire().point(latitude: MyAppState.selectedPosotion.location?.latitude ?? 0.0, longitude: MyAppState.selectedPosotion.location?.longitude ?? 0.0);
    String field = 'g';

    Stream<List<DocumentSnapshot>> stream = Geoflutterfire().collection(collectionRef: collectionReference).within(center: center, radius: double.parse((sectionConstantModel?.nearByRadius ?? 10).toString()), field: field, strictMode: true);
  ...
  Widget buildAllRestaurantsData(VendorModel vendorModel) {
    ...
    offerList.forEach((element) {
      ...
      if (vendorModel.id == element.storeId && (element.expireOfferDate?.toDate().isAfter(DateTime.now()) ?? false)) {
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          child: Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Text(
                              discountAmountTempList.reduce(min).toStringAsFixed(currencyData?.decimal ?? 2) + "% off",
